...
VALUES
    ...
    (5, 4, 'option_1', 'Read the diary.', '((event) => {showPopupMenu(event)'; TRUE)
    ...
    (11, 4, 'option_1', 'Read the diary.','((event) => {showPopupMenu(event)'; TRUE)
    ...
    (18, 4,  '(checkInventory("Mysterious Earpiece", 1) == true)',
     '((event) => {addToDetails("<br> The door is sealed shut, prehaps there is a clue of what to do somewhere in this room")})','option_1', 'The device in your ear had whirred into life when the gurgling began. Bang your pickaxe against the remants of the boulder.','(() => {loadNextDecision(4, 19)})', TRUE),
    (18, 4, 'option_2', 'Walk further into the room.', '(() => {loadNextDecision(4, 21)})', TRUE),

    (19, 4, 'option_1', 'Bang your pickaxe again.', '(() => {loadNextDecision(4, 20)})', TRUE),

    (20, 4, 'option_1', 'Go further into the chamber.','(() => {loadNextDecision(4, 21)})', TRUE),
    (20, 4, 'option_2', 'Run out of the mound.', '(() => {loadNextDecision(4, 39)})', TRUE),
    ...
    (26, 4, 'option_2', 'One of the glowing mushrooms.', '(() => {loadNextDecision(4, 29)})', TRUE),
    ...
    (31, 4, 'option_1', 'Go further into the passage.', '(() => {loadNextDecision(4, 32)})', TRUE),
    (31, 4, 'option_2', 'RUN.', '(() => {loadNextDecision(4, 40)})', TRUE),
    ...
    (33, 4, 'option_1', 'Pick up the slabs.', '(() => {loadNextDecision(4, 34)})', TRUE);
    (33, 4, 'option_2', 'Refuse to play.', '(() => {loadNextDecision(4, 41)})', TRUE),

    (34, 4, 'option_1', 'Play the game.', '(() => {loadNextDecision(4, 35)})', TRUE);
